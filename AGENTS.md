...
- Global tools on PATH (not currently in the venv): `run` (from `toml-run`, also in the dev group), `zensical`, `yamllint`. Re-run `python -m pip install --group dev` after editing the group to pick up new members.
